...
import {setArticlesArchive, deleteArticle, setModalStatus, setFilterValue} from '../../store/slices/articlesSlice'
    ...
    return (
        <div className="flex flex-wrap justify-between items-center">
            <div className="flex items-center">
                <h4 className="text-2xl font-semibold text-gray-500 dark:text-gray-100 ml-3 select-none">لیست مقالات</h4>
            ...
            <div className="block md:flex items-center w-full md:w-auto md:space-x-reverse space-x-0 md:space-x-2">
                {selectedRows.length ? <button onClick={multiDeleteHandler} className="my-3 md:my-0 w-full md:w-auto order-1 md:order-2 bg-red-600 hover:bg-red-700 focus:outline-none text-white px-3 py-2 rounded-lg duration-300">حذف {selectedRows.length} مقاله</button> : null}
                <button onClick={() => dispatch(setModalStatus(true))} className="my-3 md:my-0 w-full md:w-auto order-1 md:order-2 bg-indigo-700 hover:bg-indigo-800 focus:outline-none text-white px-3 py-2 rounded-lg duration-300">مقاله جدید</button>
                <button onClick={dataFactoryHandler} className="my-3 md:my-0 w-full md:w-auto order-1 md:order-2 bg-gray-600 hover:bg-gray-700 focus:outline-none text-white px-3 py-[0.70rem] md:py-2 rounded-lg duration-300">مقالات فیک</button>
            </div>
        </div>
    )
}
