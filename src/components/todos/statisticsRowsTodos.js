...
const StatisticsRowsTodos = () => {

    const {todosCurrentPage: todos, pagination: {totalCount, pageSize, currentPage}} = useSelector(state => state.todos)
                ...
                totalCount > pageSize
                    ?
                        <div className="text-gray-500 dark:text-gray-300 text-center md:text-right space-x-reverse space-x-1 pt-2">
                            ...
                            <span>تا سطر</span>
                                <span className="font-semibold">{(hasNext ? (pageSize * currentPage) : ((pageSize * (currentPage - 1)) + 1) + todos.length - 1).toLocaleString('fa-IR') }</span>
                            <span>از</span>
