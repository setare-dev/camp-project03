            ...
            <div className="flex justify-center pt-4 px-4 pb-20 text-center sm:block">
                ...
                <div
                    ...
                    <form onSubmit={submitHandler}>
                        <MultiColumnElement>
                            <InputElement label="نام" keyname="name" value={data.name} error={errors.name}
                                          inputHandler={inputHandler}/>
                            <InputElement label="فامیل" keyname="family" value={data.family} error={errors.family}
                                          inputHandler={inputHandler}/>
                        </MultiColumnElement>

                        <MultiColumnElement>
                            <DateBirthElement label="متولد" day={data.day} month={data.month} year={data.year}
                                              errors={errors} inputHandler={inputHandler}/>
                            <SelectElement label="جنسیت" keyname="gender" value={data.gender}
                                           options={[{value: 0, text: 'مرد'}, {value: 1, text: 'زن'}]}
                                           error={errors.gender} inputHandler={inputHandler}/>
                        </MultiColumnElement>

                        <MultiColumnElement>
                            <InputElement label="ایمیل" keyname="email" value={data.email} error={errors.email}
                                          inputHandler={inputHandler} dir="ltr"/>
                            <SelectElement label="نوع کاربری" keyname="isAdmin" value={data.isAdmin}
                                           options={[{value: 0, text: 'معمولی'}, {value: 1, text: 'مدیر'}]}
                                           error={errors.isAdmin} inputHandler={inputHandler}/>
                        </MultiColumnElement>

                        <div className="text-left mt-8">
                            <button type="submit"
                                    className="lose justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-700 text-base font-medium text-white hover:bg-green-800 focus:outline-none ml-3 sm:w-auto sm:text-sm focus:bg-green-800">{state.userIdForUpdate ? 'ویرایش' : 'ثبت'}</button>
                            <button onClick={cancelHandler} type="button"
                                    className="lose justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none sm:w-auto sm:text-sm focus:bg-red-800">انصراف
                            </button>
                        </div>
                    </form>
                </div>
