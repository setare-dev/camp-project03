...
const HeaderUsers = ({viewType, changeViewType}) => {
    ...
    const {users: {data: users}, global: {filterValue, selectedRows, pagination: {totalCount, pageSize, currentPage}}} = useSelector(state => state)
    ...
    const multiDeleteHandler = async () => {
        try {
            ...
            if (result) {
                ...
                const {data: {data, meta: {totalDocs, limit, page}}} = await getUsersService(currentPage < lastPage ? currentPage : selectedRows.length < users.length ? currentPage : currentPage - 1, filter)
    ...
    return (
        <div className="flex flex-wrap justify-between items-center">
            ...
            <div className="block md:flex items-center w-full md:w-auto md:space-x-reverse space-x-0 md:space-x-2">
                {selectedRows.length ? <button onClick={multiDeleteHandler} className="my-3 md:my-0 w-full md:w-auto order-1 md:order-2 bg-red-600 hover:bg-red-700 focus:outline-none text-white px-3 py-[0.70rem] md:py-2 rounded-lg">حذف {selectedRows.length} کاربر</button> : null}
